...
```{r}
shannon <- wide_df |> 
  filter(Donor == "Fresh" & Source != "Fecal" & Diet %in% c("Western", "LFPP")) |>#first generation ("fresh" donor), only take into account "final" snapshots of the microbiota (no intermediate fecal samples), and we make sure we are only comparing the two diets 
  group_by(Diet) |> 
